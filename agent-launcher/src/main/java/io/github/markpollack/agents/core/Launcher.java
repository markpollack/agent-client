...
public class Launcher {
	...
	private static Map<String, AgentRunner> createTempAgentRegistry(String agentId) {
		...
		if ("hello-world".equals(agentId)) {
			try {
				Class<?> agentClass = Class.forName("io.github.markpollack.agents.helloworld.HelloWorldAgentRunner");
			...
			try {
				Class<?> agentClass = Class
					.forName("io.github.markpollack.agents.helloworldai.HelloWorldAgentAIRunner");
				AgentRunner agent = (AgentRunner) agentClass.getDeclaredConstructor().newInstance();
			...
			try {
				Class<?> agentClass = Class
					.forName("io.github.markpollack.agents.codecoverage.CodeCoverageAgentRunner");
				AgentRunner agent = (AgentRunner) agentClass.getDeclaredConstructor().newInstance();
